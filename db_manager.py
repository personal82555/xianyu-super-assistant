...
class DBManager:
    ...
    def init_db(self):
        ...
        try:
            ...
            CREATE TABLE IF NOT EXISTS ai_reply_settings (
                ...
                custom_prompts TEXT,
                refund_policy TEXT DEFAULT 'allow',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (cookie_id) REFERENCES cookies(id) ON DELETE CASCADE
            )
            ...
            ('ai_default_enabled', 'false', 'AI回复-默认启用'),
            ('ai_default_model', 'qwen-plus', 'AI回复-默认模型'),
            ('ai_default_base_url', 'https://dashscope.aliyuncs.com/compatible-mode/v1', 'AI回复-默认API地址'),
            ('ai_default_api_key', '', 'AI回复-默认API密钥'),
            ('ai_default_max_discount_percent', '10', 'AI回复-默认最大优惠百分比'),
            ('ai_default_max_discount_amount', '100', 'AI回复-默认最大优惠金额'),
            ('ai_default_max_bargain_rounds', '3', 'AI回复-默认最大议价轮数'),
            ('ai_default_prompts', '', 'AI回复-默认自定义提示词'),
            ('ai_default_refund_policy', 'allow', 'AI回复-默认退款策略(allow/disallow)')
            ''')
    ...
    def check_and_upgrade_db(self, cursor):
        ...
        try:
            ...
            if current_version < "1.4":
                ...
                if 'refund_policy' not in ai_columns:
                    cursor.execute("ALTER TABLE ai_reply_settings ADD COLUMN refund_policy TEXT DEFAULT 'allow'")
                    logger.info("添加ai_reply_settings表的refund_policy列")
            ...
            if current_version < "1.5":
                ...
                if 'role' not in user_columns:
                    ...
                    logger.info("添加users表的role列，并将admin用户设为admin角色")
        ...
        with self.lock:
            try:
                ...
                (cookie_id, ai_enabled, model_name, api_key, base_url,
                 ...
                 custom_prompts, refund_policy, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                ''', (
                    cookie_id,
                    ...
                    settings.get('refund_policy', 'allow')
                ))
                self.conn.commit()
            ...
            try:
                ...
                if result:
                    return {
                        ...
                        'refund_policy': result[8] or 'allow'
                    }
                else:
                    # 使用系统全局默认设置
                    return self.get_ai_default_settings()
            except Exception as e:
    ...
    def get_ai_default_settings(self) -> dict:
        """获取AI回复的全局默认设置（从 system_settings 表中读取）"""
        all_settings = self.get_all_system_settings()
        return {
            'ai_enabled': all_settings.get('ai_default_enabled', 'false').lower() == 'true',
            'model_name': all_settings.get('ai_default_model', 'qwen-plus'),
            'api_key': all_settings.get('ai_default_api_key', ''),
            'base_url': all_settings.get('ai_default_base_url', 'https://dashscope.aliyuncs.com/compatible-mode/v1'),
            'max_discount_percent': int(all_settings.get('ai_default_max_discount_percent', '10')),
            'max_discount_amount': int(all_settings.get('ai_default_max_discount_amount', '100')),
            'max_bargain_rounds': int(all_settings.get('ai_default_max_bargain_rounds', '3')),
            'custom_prompts': all_settings.get('ai_default_prompts', ''),
            'refund_policy': all_settings.get('ai_default_refund_policy', 'allow')
        }
    ...
    def create_user(self, username: str, email: str, password: str, role: str = 'user') -> bool:
        ...
        with self.lock:
            try:
                ...
                self.conn.commit()
                logger.info(f"创建用户成功: {username} ({email}), 角色: {role}")
                return True
    ...
    def get_user_by_username(self, username: str) -> Optional[Dict[str, Any]]:
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                cursor.execute('''
                SELECT id, username, email, password_hash, is_active, created_at, updated_at, COALESCE(role, 'user') as role
    ...
    def update_user_email(self, username: str, new_email: str) -> bool:
        ...
        with self.lock:
            try:
                ...
                if cursor.rowcount > 0:
                    self.conn.commit()
                    logger.info(f"用户 {username} 邮箱更新成功: {new_email}")
                    return True
                return False
            except Exception as e:
    ...
    def update_user_role(self, username: str, new_role: str) -> bool:
        ...
        with self.lock:
            try:
                ...
                if cursor.rowcount > 0:
                    self.conn.commit()
                    logger.info(f"用户 {username} 角色更新成功: {new_role}")
                    return True
                return False
            except Exception as e:
...
© 2025 闲鱼自动回复系统"""
    ...
    def update_item_fields(self, cookie_id: str, item_id: str,
                           item_title: str = None, item_price: str = None,
                           item_description: str = None, item_detail: str = None) -> bool:
        """更新商品多个字段
        ...
        try:
            with self.lock:
                ...
                if item_title is not None:
                    set_fields.append("item_title = ?")
                    params.append(item_title)
                if item_price is not None:
                    set_fields.append("item_price = ?")
                    params.append(item_price)
                if item_description is not None:
                    set_fields.append("item_description = ?")
                    params.append(item_description)
                if item_detail is not None:
                    set_fields.append("item_detail = ?")
                    params.append(item_detail)

                if not set_fields:
                    logger.warning(f"没有要更新的字段: {item_id}")
                    return False

                set_fields.append("updated_at = CURRENT_TIMESTAMP")
                params.extend([cookie_id, item_id])

                sql = f'''UPDATE item_info SET
                    {', '.join(set_fields)}
                    WHERE cookie_id = ? AND item_id = ?'''
                ...
                if cursor.rowcount > 0:
                    self.conn.commit()
                    logger.info(f"更新商品字段成功: {item_id} ({len(set_fields)}个字段)")
                    return True
                else:
                    logger.warning(f"未找到商品: {item_id}")
                    return False
        except Exception as e:
            logger.error(f"更新商品字段失败 {item_id}: {e}")
            return False
    ...
    def get_all_users(self):
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                cursor.execute('''
                SELECT id, username, email, created_at, updated_at, COALESCE(role, 'user') as role
                FROM users
                ORDER BY created_at DESC
                ''')
    ...
    def get_ai_conversations(self, cookie_id: str = None, user_id: str = None,
                              item_id: str = None, page: int = 1, page_size: int = 20,
                              user_cookie_ids: list = None) -> dict:
        """分页查询AI对话历史
        ...
        Args:
            ...
            page: 页码，从1开始
            page_size: 每页条数，默认20
            user_cookie_ids: 当前用户可访问的cookie_id列表（权限过滤）
        """
        with self.lock:
            try:
                cursor = self.conn.cursor()
                ...
                if user_cookie_ids:
                    placeholders = ','.join('?' * len(user_cookie_ids))
                    conditions.append(f'c.cookie_id IN ({placeholders})')
                    params.extend(user_cookie_ids)

                if cookie_id:
                    conditions.append('c.cookie_id = ?')
                ...
                if item_id:
                    conditions.append('c.item_id = ?')
                    params.append(item_id)

                where_clause = ' AND '.join(conditions) if conditions else '1=1'

                # 查询总数
                count_sql = f'SELECT COUNT(*) FROM ai_conversations c WHERE {where_clause}'
                ...
                data_sql = f'''
                    SELECT c.id, c.cookie_id, c.chat_id, c.user_id, c.item_id,
                           c.role, c.content, c.intent, c.created_at
                ...
                columns = ['id', 'cookie_id', 'chat_id', 'user_id', 'item_id',
