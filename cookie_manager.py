...
class CookieManager:
    ...
    def _load_from_db(self):
        ...
        try:
            ...
            for cookie_id in self.cookies.keys():
                ...
                # 初始化在线状态为False
                self.online_status[cookie_id] = False
    ...
    def update_online_status(self, cookie_id: str, is_online: bool):
        """更新指定Cookie的WebSocket在线状态（由XianyuLive调用）"""
        self.online_status[cookie_id] = is_online
        status_text = "在线" if is_online else "离线"
        logger.debug(f"更新在线状态: {cookie_id} -> {status_text}")

    def get_online_status(self, cookie_id: str) -> Optional[bool]:
        """获取指定Cookie的在线状态，None表示未知"""
        return self.online_status.get(cookie_id)

    def get_all_online_status(self) -> Dict[str, bool]:
        """获取所有Cookie的在线状态"""
        return dict(self.online_status)
